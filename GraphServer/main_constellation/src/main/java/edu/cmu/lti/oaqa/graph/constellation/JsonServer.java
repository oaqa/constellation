...
public class JsonServer {
    ...
    public static class DemoGraphHandler implements RequestHandler {
    	...
    	private RexsterGraph graph = new RexsterGraph("http://peace.isri.cs.cmu.edu:8182/graphs/CMUGraph");
	...
	public JSONRPC2Response process(JSONRPC2Request req, MessageContext ctx) {
		...
		//get the first thing that says text
		//look for the first thing that says traverse
		//get the next thing and look for the 2nd thing in the array
				
		JSONArray searchTextArray = (JSONArray) query.get(0);//getFirstQueryObjectWithLabel("text",query);
		System.out.println("searchTextArray: "+searchTextArray);
		//if(searchTextArray==null) return new JSONRPC2Response(new Error());
		String searchText = searchTextArray.get(1).toString();
		
		System.out.println("here 4");
		
		JSONArray propertyTextArray = (JSONArray) query.get(3);
		System.out.println("propertyTextArray: "+propertyTextArray);
		//if(propertyTextArray==null) return new JSONRPC2Response(new Error());
		String propertyText = propertyTextArray.get(1).toString();
		...
		*/
		...
		if(searchText!=null) searchText=searchText.toLowerCase().replaceAll(" ", "_");
		//if(traverseString!=null) traverseString=traverseString.toLowerCase();
		if(propertyText!=null) propertyText=propertyText.toLowerCase();
		...
		*/
		...
		//do graph lookup
		Vertex vertex = findVertex(graph,searchText);
		String propertyResult = vertex.getProperty(propertyText.toString()).toString(); //getProperty(vertex, propertyText);
